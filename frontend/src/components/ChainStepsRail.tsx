...
// Horizontal step indicator for chain runs. Lifted out of AgentCard so
// both the card and the detail page can render it without duplication.
// When `linkable` is true each step links to its own detail page.
export default function ChainStepsRail({ steps, currentRunID, linkable }: Props) {
  ...
  const currentIndex = steps.findIndex((s) => s.ID === currentRunID)
    ...
    <div>
      <div className="flex items-center gap-2">
        {steps.map((step, i) => {
          ...
          const isFailed = ['failed', 'cancelled', 'task_unsolvable'].includes(step.Status)
          const isCurrent = step.ID === currentRunID
          const node = (
