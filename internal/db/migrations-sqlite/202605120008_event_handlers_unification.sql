...
CREATE TABLE runs_new (
    ...
    -- Preserve chain columns added by 202605110002_prompt_chains: the
    -- rebuild here is purely to repoint trigger_id's FK, not to drop
    -- chain wiring.
...
-- Restore the chain lookup index dropped along with the original runs table
-- (originally created in 202605110002_prompt_chains).
...
-- ============================================================================
-- (4b) Rebuild chain_runs to repoint trigger_id at event_handlers(id).
--      chain_runs was added by 202605110002_prompt_chains with an FK to
--      prompt_triggers; the same rebuild-dance pattern as runs above
--      moves it onto event_handlers so the parent drop in (5) doesn't
--      leave a dangling FK target.
-- ============================================================================

CREATE TABLE chain_runs_new (
    id              TEXT PRIMARY KEY,
...
DROP TABLE chain_runs;
ALTER TABLE chain_runs_new RENAME TO chain_runs;
