...
-- chain_runs.trigger_id was added by 202605110002_prompt_chains with an FK
-- to prompt_triggers; repoint it to event_handlers so the parent drop below
-- isn't blocked. The constraint name follows the Postgres auto-generated
-- pattern (table_col_col_fkey) for the composite (trigger_id, org_id).
ALTER TABLE chain_runs DROP CONSTRAINT IF EXISTS chain_runs_trigger_id_org_id_fkey;
ALTER TABLE chain_runs
  ADD CONSTRAINT chain_runs_trigger_id_org_id_fkey
  FOREIGN KEY (trigger_id, org_id) REFERENCES event_handlers (id, org_id);
