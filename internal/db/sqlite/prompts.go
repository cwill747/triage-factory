...
func (s *promptStore) Create(ctx context.Context, orgID string, p domain.Prompt) error {
	...
	_, err := s.q.ExecContext(ctx, `
		INSERT INTO prompts (id, name, body, source, kind, allowed_tools, model, usage_count, team_id, visibility, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, 'team', ?, ?)
	`, p.ID, p.Name, p.Body, p.Source, kind, p.AllowedTools, p.Model, runmode.LocalDefaultTeamID, now, now)
	return err
}
